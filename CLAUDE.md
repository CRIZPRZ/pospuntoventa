...
- **Reintentos ante hipos de DNS de Docker en Baileys (2026-08-08)**: `BaileysWhatsAppService::request()` agrega `->retry(3, fn($attempt) => $attempt * 500)` al cliente HTTP. Causa raíz: `docker exec ventas-app curl http://whatsapp-baileys:3025/health` respondía perfecto (misma red `pos_ventas-network`), pero ocasionalmente fallaba con `cURL error 6: Could not resolve host` desde la app real — hipo transitorio del DNS embebido de Docker, no un problema de config. `retry()` de Laravel solo reintenta excepciones de conexión (no respuestas HTTP 4xx/5xx, esas las sigue manejando `$response->failed()`), así que no reintenta errores reales del lado de Baileys.
- **`planes.periodicidad` (2026-08-07)**: columna nueva (`mensual` default | `unico`), solo relevante para `tipo='manual'` — los `stripe` siempre son recurrentes. `Plan::esPagoUnico()`. Antes de esto, el frontend mostraba "/mes" en TODOS los planes sin importar si eran pago único, incluidos los manuales negociados por transferencia. Planes manuales creados antes de esta migración quedan en `mensual` por default y hay que corregirlos manualmente en superadmin si eran de pago único.
...
- No es módulo de plan — siempre incluido en todos los planes (Trial/Básico/Pro/Ilimitado), el negocio lo activa/desactiva por su cuenta. No agregar gate `can:` ni entrada en `modulosDefault()`/`MODULOS_TRIAL`.
- Config vive en `configuraciones.config->loyalty` (mismo mecanismo que `whatsapp`/`pos`), sección agregada a `EMPRESA_SECTIONS` en `ConfiguracionController` — por eso es a nivel empresa, no por sucursal (el saldo de puntos del cliente ya es a nivel empresa vía `Cliente::empresa_id`). Shape: `{ activo, modo: 'total'|'producto', monto_por_punto, puntos_otorgados, valor_punto }`. Defaults en `ConfiguracionController::defaultsEmpresa()`.
- `LoyaltyService` (`app/Services/LoyaltyService.php`) centraliza cálculo/aplicación: `calcularPuntosGanados()`, `calcularDescuentoPuntos()` (solo valida y calcula, no muta), `registrarGanados()`/`aplicarCanje()` (mutan saldo + insertan ledger).
- Modo `total`: puntos = `floor(total_venta / monto_por_punto) * puntos_otorgados`. Modo `producto`: por línea, usa `productos.puntos_por_unidad` si el producto lo tiene asignado (columna nullable), si no cae al mismo cálculo que modo `total` pero por subtotal de línea.
- `loyalty_transactions` es ledger append-only (`type`: `earned`/`redeemed`/`adjusted`) — fuente de verdad histórica. `clientes.points_balance`/`lifetime_points` son contadores denormalizados, se mantienen en sync dentro de la misma transacción SQL de la venta.
- Canje es de un solo paso (sin QR todavía — ver nota abajo): el frontend manda `puntos_canjeados` en el payload de `POST /api/ventas`, `VentaController@store` valida saldo, calcula descuento (`puntos * valor_punto`), lo resta del total y lo suma a `descuento` antes de guardar la venta. Todo dentro de la misma `DB::transaction` que crea venta/items/pagos.
- `VentaController@cancelar` revierte: descuenta `puntos_ganados` de esa venta del balance/lifetime, regresa `puntos_canjeados`, e inserta un registro `adjusted` en el ledger. Clamp a 0 — nunca deja `points_balance` negativo aunque el cliente ya haya gastado esos puntos en otra venta.
- Endpoint `GET /api/clientes/{id}/puntos` — saldo + últimos 50 movimientos del ledger, mismo grupo de permiso `can:ver clientes`.
- **Sin QR todavía**: el canje es manual, el cajero lo aplica directo en el POS (no hay tabla `loyalty_redemptions` ni código temporal). Cuando exista módulo de tienda online, se puede agregar una tabla de canjes con `code`/`status pending`/`expires_at` para que el cliente genere el QR desde su portal — investigado el patrón completo en `pos-fenicia` (proyecto hermano) si se retoma esto: doble QR (identidad autofirmada HMAC sin hit a BD + canje opaco con lookup en BD y reserva en dos fases antes de debitar puntos).
- **Bug resuelto: puntos no prorrateados (2026-08-19)**: `calcularPuntosGanados()` usaba `floor(total/monto_por_punto)*puntos_otorgados` — con `monto_por_punto=100, puntos_otorgados=10`, una venta de $20 daba 0 puntos (no llegaba al umbral completo) en vez de prorratear. Fix: `floor(total*puntos_otorgados/monto_por_punto)` — mismo patrón usado por apps reales de lealtad (Fallabella, Farmacias del Ahorro), acredita proporcional a lo gastado sin esperar a completar el bloque. Aplica igual al fallback del modo `producto`.
- **Ticket WhatsApp con puntos (2026-08-19)**: `VentaTicketWhatsAppMessage::build()` recibe un tercer parámetro opcional `?Cliente $cliente` (el `venta->cliente` original, no el `cliente_id` que el cajero puede pasar solo para reenviar a otro teléfono — evita mostrar el saldo de un cliente distinto al que ganó/canjeó los puntos). Si la venta tuvo `puntos_ganados`/`puntos_canjeados` > 0, agrega al mensaje: puntos canjeados, puntos ganados y **puntos disponibles actuales** (el saldo ya refleja la venta, porque el canje/acumulación ya se aplicó en `VentaController@store` antes de que el cajero confirme el envío por WhatsApp).
