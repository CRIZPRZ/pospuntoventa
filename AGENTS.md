...
- **Licencia desktop fase 1 (2026-08-04)**: el instalable Windows no debe inventar otro sistema comercial; cuelga de `empresas.plan_estado` y `plan_vigente_hasta`. El backend expone `GET /api/desktop/license`, `POST /api/desktop/license/activate`, `POST /api/desktop/license/deactivate-device` con `auth:sanctum`, y `POST /api/desktop/license/validate` con `license_key + device_uuid`.
...
- **Electron bootstrap contract (2026-08-04)**: el shell de escritorio ya consume `POST /api/desktop/license/activate` al iniciar sesión y espera recibir `license.resolved_status`, `access.allowed`, `license.key`, `license.grace_until` y límites. Mantener estable ese shape para no romper el instalable.
