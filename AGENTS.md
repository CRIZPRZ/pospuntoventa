...
- **Licencia desktop fase 1 (2026-08-04)**: el instalable Windows no debe inventar otro sistema comercial; cuelga de `empresas.plan_estado` y `plan_vigente_hasta`. El backend expone `GET /api/desktop/license`, `POST /api/desktop/license/activate`, `POST /api/desktop/license/deactivate-device` con `auth:sanctum`, y públicas `POST /api/desktop/license/validate` (`license_key + device_uuid`) y `POST /api/desktop/license/activate-device` (`license_key + email + device_uuid...`, ver fase pre-login abajo).
- **Licencia desktop — activación PRE-LOGIN (2026-08-05)**: `DesktopLicenseController::activateByEmail` + `DesktopLicenseService::activateDeviceByEmail` resuelven la empresa por `license_key` (sin sesión) y validan que `email` sea un usuario con rol `admin` de esa empresa. Ruta pública `desktop/license/activate-device` con `throttle:desktop-activate` (limiter nuevo en `AppServiceProvider`, mismo patrón que `register`). Errores devuelven `{message, code}` vía `App\Exceptions\DesktopLicenseException` (tiene `render()` propio) con códigos `invalid_license_key|license_suspended|license_cancelled|license_expired|email_mismatch|max_devices_reached`. `DesktopLicenseService::validateDevice` también devuelve códigos `device_not_registered`/`device_revoked` ahora, en vez del `ValidationException` genérico anterior — el frontend usa esos códigos para forzar reactivación.
- **`licenses.owner_user_id` (2026-08-05)**: nueva columna nullable FK a `users`, `nullOnDelete` (migración `2026_08_05_000001_add_owner_user_id_to_licenses_table`). `License::owner()` (BelongsTo). Se expone como `license.owner: {id,name,email}|null` en `DesktopLicenseService::buildResponse()`. `SuperAdminEmpresaController::updateLicense` acepta `owner_user_id` (valida que pertenezca a la empresa) y nuevo `SuperAdminEmpresaController::usuarios()` + ruta `GET superadmin/empresas/{empresa}/usuarios` lista usuarios de la empresa para el selector del frontend.
- **Modo offline controlado (2026-08-04)**: `DesktopLicenseService` renueva `grace_until` en cada activación/validación usando `DESKTOP_LICENSE_GRACE_HOURS` (default `48`). Si el plan ya no está vigente pero `grace_until` sigue vivo, la licencia responde `resolved_status='grace'` y permite operación temporal.
- **Dispositivos por licencia (2026-08-04)**: una empresa tiene una sola fila en `licenses` y múltiples `license_devices`. `DESKTOP_DEFAULT_MAX_DEVICES` define el tope inicial; si se excede, `activate` responde 422. No duplicar el contador en frontend: la fuente de verdad es backend.
- **Licencia desktop fase 2 (2026-08-04)**: superadmin gestiona la licencia por empresa desde `GET /api/superadmin/empresas/{empresa}/license`, `PUT /api/superadmin/empresas/{empresa}/license` y `POST /api/superadmin/empresas/{empresa}/license/devices/{deviceUuid}/revoke`.
- **Auto-actualización del `.exe` con `electron-updater` (2026-08-08)**: el frontend Electron ahora revisa actualizaciones apuntando a `https://api.eventpos.online/storage/desktop` (provider `generic`, mismo disco que ya servía el instalador manual). `DesktopInstallerController::upload` cambió de aceptar un campo único `installer` a un array `files[]` — el superadmin sube `.exe` + `latest.yml` + `.exe.blockmap` juntos (los 3 los genera `electron-builder` al compilar gracias al `publish` config). `DesktopInstallerService::store()` reemplaza al viejo `replace()`: antes borraba TODO `storage/app/public/desktop/` en cada subida (habría borrado `latest.yml`/`.blockmap` al subir solo el `.exe`), ahora solo borra `.exe` viejos y deja intactos los demás archivos del set de release. `isAllowed()` valida extensión (`exe|yml|yaml|blockmap`) por archivo antes de guardar cualquiera.
- **Envío de instalador desktop por correo (2026-08-05)**: `App\Services\DesktopInstallerService` gestiona el único `.exe` vigente en `storage/app/public/desktop/` (mismo disco/patrón que `ConfiguracionController::uploadLogo()`), sin tabla nueva — deriva `filename/size/uploaded_at` del propio archivo, se queda con el más reciente si hubiera varios. `SuperAdmin\DesktopInstallerController` expone `GET/POST /api/superadmin/desktop-installer` (subida global, no por tenant — el instalador es el mismo binario para todas las empresas). `SuperAdminEmpresaController::sendInstallerEmail` (`POST /api/superadmin/empresas/{empresa}/license/send-installer`, body `{email}`) resuelve la licencia vía `DesktopLicenseService::getOrCreateForEmpresa()`, responde 422 si no hay instalador subido, y encola `App\Mail\DesktopInstallerMail` (patrón de `App\Mail\WelcomeMail`: Mailable moderno `envelope()`/`content()`, mailer global vía `Mail::to()->queue()`). **No usa `EnviaCorreosTrait`** — ese trait exige `tenant_id` resuelto vía middleware y este envío lo dispara superadmin sin contexto de tenant. Vista `resources/views/emails/desktop-installer.blade.php` extiende `emails.layout` (mismo wrapper que `welcome.blade.php`).
- **Controles administrativos desktop (2026-08-04)**: `updateLicense` solo cambia `status` (`active`, `suspended`, `cancelled`) y `max_devices`. `revokeLicenseDevice` desactiva un equipo existente sin borrar historial; la app instalada debe volver a activarse si quiere regresar.
- **Electron bootstrap contract (2026-08-04, actualizado 2026-08-05)**: el shell de escritorio ahora activa el equipo ANTES del login vía `POST /api/desktop/license/activate-device` (público); `POST /api/desktop/license/activate` (con sesión) queda como fallback legado, ya no es el flujo normal. Ambos esperan recibir `license.resolved_status`, `access.allowed`, `license.key`, `license.grace_until`, `license.owner` y límites. Mantener estable ese shape para no romper el instalable.
- **Desktop validate contract (2026-08-04)**: el frontend/Electron ya usa también `POST /api/desktop/license/validate` para refresco periódico. Esa ruta debe seguir aceptando `license_key`, `device_uuid` y `fingerprint`, y responder el mismo shape base de licencia que `activate`.
- **Activación desktop manual (2026-08-04)**: toda licencia nueva nace con `status='suspended'`, tanto por `DesktopLicenseService::getOrCreateForEmpresa()` como por default de la tabla. `activate` debe responder el estado bloqueado sin crear `license_devices`; solo superadmin puede cambiarla a `active`.
- **Suspensión en vivo (2026-08-04)**: el frontend revalida cada 60 segundos y al recuperar foco. `validate` debe resolver `suspended` antes que plan/gracia para bloquear una sesión abierta sin reiniciar y nunca devolver un motivo viejo de límite de equipos.
- **Primera activación pendiente (2026-08-04)**: una licencia nueva suspendida responde con llave pero `device=null`. El frontend continúa llamando `activate` hasta que superadmin cambie el estado a `active`; recién después de vincular el equipo pasa a `validate`.
...
- **Módulos fijos por plan (2026-07-30)**: `PlanesSeeder` excluye Cámaras de todos los planes, agrega WhatsApp a Pro e Ilimitado y sincroniza empresas activas con planes Stripe existentes. Planes manuales conservan su configuración, salvo que Cámaras se desactiva globalmente. Trial y defaults de empresa no incluyen Cámaras ni WhatsApp.
...
## Planes — periodicidad (2026-08-07)
- `planes.periodicidad` (`mensual` default | `unico`) nueva, solo aplica a `tipo='manual'`. `Plan::esPagoUnico()`. Frontend (`MiPlan.jsx`, `Empresas.jsx` superadmin) muestra "(pago único)" en vez de "/mes" cuando corresponde. Correo de soporte en emails/UI cambiado de `soporte@eventpos.online` a `c.lira.prz@gmail.com`.
...
- **Bug resuelto: puntos no prorrateados (2026-08-19)**: fórmula corregida a `floor(total*puntos_otorgados/monto_por_punto)` en vez de `floor(total/monto_por_punto)*puntos_otorgados` — la versión vieja daba 0 puntos en compras menores al umbral completo (ej. $20 con regla "100→10" no daba nada). Ahora acredita proporcional, igual que apps de lealtad reales.
- **Ticket WhatsApp con puntos (2026-08-19)**: `VentaTicketWhatsAppMessage::build($venta, $businessName, $cliente)` — tercer parámetro es siempre `$venta->cliente` (no el `cliente_id` opcional que el cajero puede pasar solo para redirigir el envío a otro teléfono). Si la venta ganó o canjeó puntos, el mensaje incluye puntos canjeados, puntos ganados y saldo disponible actual.
