...
class ReportesController extends Controller
{
    public function general(Request $request)
    {
        ...
        $ventasCanceladas = Venta::whereBetween('created_at', [$inicio, $fin])
            ->where('estado', 'cancelada')
            ->get();
        ...
        $totalCosto = VentaItem::whereIn('venta_id', $ventas->pluck('id'))
            ->get()
            ->sum(function ($item) {
                return ($item->costo_unitario ?? 0) * $item->cantidad;
            });
        ...
        $valorInventario = $productos->sum(function ($p) {
            return ($p->costo ?? 0) * ($p->stock ?? 0);
        });
        $valorInventarioVenta = $productos->sum(function ($p) {
            return ($p->precio ?? 0) * ($p->stock ?? 0);
        });
        $totalProductos = $productos->sum('stock');
        $productosBajoStock = $productos->filter(function ($p) {
            return $p->control_stock && $p->stock <= $p->stock_minimo;
        })->count();
        $productosSinStock = $productos->filter(function ($p) {
            return $p->control_stock && $p->stock <= 0;
        })->count();

        $clientes = Cliente::where('activo', true)->get();
        $totalCredito = $clientes->sum('saldo_credito');
        $clientesConDeuda = $clientes->filter(fn ($c) => (float) $c->saldo_credito > 0)->count();
        $totalAbonos = Abono::whereBetween('created_at', [$inicio, $fin])->sum('monto');
